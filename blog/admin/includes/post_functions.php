...
function createPost($request_values)
	...
	* * * * * * * * * * * * * * * * * * * * * */
	function editPost($role_id)
	{
		...
		$post_ID =  $post['id'];
		$topic_id = $conn->query("Select topic_id from post_topic where post_id =".$post_ID);
		$title = $post['title'];
		$body = $post['body'];
	...
	function updatePost($request_values)
	{
		...
		// create slug: if title is "The Storm Is Over", return "the-storm-is-over" as slug
		$post_slug = makeSlug($title);
///////////////// bu bölümü ekledim şuan fotoğraf güncelliyor.
		global $conn;
		$image = "SELECT 'image' FROM posts";
		$featured_image = $_FILES['featured_image']['name'];
		if (empty($featured_image)) { //array_push($errors, "Featured image is required"); 
...
/////////////////
		...
		if (count($errors) == 0) {
			...
			$_SESSION['message'] = "Post updated successfully".$_FILES['featured_image']['name'];
			header('location: posts.php');
			exit(0);
		}
	}
